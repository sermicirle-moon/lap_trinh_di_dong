...
import { View, Text, TextInput, TouchableOpacity, StyleSheet, Modal, KeyboardAvoidingView, Platform, ScrollView, Keyboard } from 'react-native';
import { Ionicons } from '@expo/vector-icons';
import ListColorPicker from './ListColorPicker';

export default function ListForm({ visible, mode = 'create', initialData = null, onSave, onCancel }) {
  ...
  useEffect(() => {
    if (visible) {
      if (mode === 'edit' && initialData) {
        setName(initialData.title || ''); setColor(initialData.color || '#2D9CDB'); setIcon(initialData.icon || 'list-outline');
      } else {
        setName(''); setColor('#2D9CDB'); setIcon('list-outline');
      }
    }
  }, [visible, mode, initialData]);

  const handleSave = () => {
    if (!name.trim()) { alert('Vui lòng nhập tên danh sách'); return; }
    Keyboard.dismiss();
    setTimeout(() => {
  ...
  return (
    <Modal visible={visible} transparent animationType="slide" onRequestClose={handleCancel}>
      <KeyboardAvoidingView style={styles.backdrop} behavior={Platform.OS === 'ios' ? 'padding' : 'height'}>
        ...
        <View style={styles.container}>
          <View style={styles.header}>
            ...
            <Text style={styles.title}>{mode === 'create' ? 'Danh sách mới' : 'Chỉnh sửa'}</Text>
          ...
          <ScrollView style={styles.content} showsVerticalScrollIndicator={false}>
            <View style={styles.section}>
              ...
              <TextInput style={styles.input} placeholder="Nhập tên..." value={name} onChangeText={setName} autoFocus placeholderTextColor="#C0C0C0" />
            </View>
            ...
            <View style={styles.section}>
              ...
              <TouchableOpacity
                ...
                onPress={() => { Keyboard.dismiss(); setShowColorPicker(true); }} // Hạ bàn phím khi chọn màu
              >
            ...
            <View style={styles.section}>
              ...
              <View style={styles.iconGrid}>
                {['list-outline', 'document', 'folder-outline', 'star-outline', 'heart-outline'].map((iconName) => (
                  <TouchableOpacity key={iconName} style={[styles.iconButton, icon === iconName && styles.iconButtonSelected]} onPress={() => { Keyboard.dismiss(); setIcon(iconName); }}>
      ...
      {/* CHUYỂN COLOR PICKER VÀO BÊN TRONG MODAL NÀY */}
      <ListColorPicker visible={showColorPicker} onClose={() => setShowColorPicker(false)} onSelectColor={setColor} currentColor={color} />
    </Modal>
  );
}

// ... Giữ nguyên toàn bộ styles cũ của ListForm.js của bạn
